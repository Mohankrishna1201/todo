...
def createTask(request: TaskCreate):
    ...
    task = request.task
    print(f"Received request with token: {token}, task: {task}")

    try:
        data, error = supabase_client.table("todos") \
            .insert({"user_id": user_id, "task": task}) \
            .execute()

        if error:
            # Handle error here (e.g., log the error, return appropriate error response)
            print(f"Failed to create task: {error}")
            return {"error": str(error)}  # Example error response

        return data[1][0]  # Return the created task data
    except Exception as e:  # Catch any other unexpected exceptions
        print(f"Unexpected error: {e}")
        return {"error": "Internal server error"}  # Generic error response
